...
def train(agent, env, e, t, train_start, epsilon, min_epsilon, anneal_epsilon, initializer, cum_losses_old, graph_learning_stop):
    ...
    while (not done) and (step < max_episode_limit):
        ...
        avail_action = env.get_avail_actions()
        n_agent = len(avail_action)
        n_f = np.array(deepcopy(node_feature))
        if cfg.given_edge == True:
            node_representation = agent.get_node_representation_temp(
                node_feature,
                agent_feature,
            ...
            node_representation, A, X = agent.get_node_representation_temp(
                ...
                dead_masking = dead_masking,
                mini_batch=False)
        #print(node_representation.shape, A.shape)

        edge_shape = torch.tensor(edge_index_enemy).shape[1]
        values = torch.ones(edge_shape)

        B = torch.sparse_coo_tensor(edge_index_enemy, values, (edge_shape, edge_shape)).long().to_dense()

        num_nodes = np.array(n_f).shape[0]
        node_labels = np.round(n_f[:, 10], decimals=1)  # Rounded to 1 decimal place
        x_positions = n_f[:, 7]
        y_positions = n_f[:, 8]
        # print(x_positions)
        # print(y_positions)
        ...
        # Color nodes based on index
        node_colors = ['blue' if i < 10 else 'red' for i in range(num_nodes)]
        ...
        for i in range(num_nodes):
            G.add_node(i, pos=(x_positions[i], y_positions[i]), label=node_labels[i])
        ...
        for i in range(edge_shape):
            for j in range(edge_shape):
                if B[i, j] == 1:
        ...
        nx.draw(
            G, pos, node_color=node_colors, with_labels=False, node_size=500,
            edge_color=edge_colors  # Use the edge colors we specified
        )
        nx.draw_networkx_labels(
            G, pos, labels={i: f"{label}" for i, label in labels.items()}, font_size=10, font_color="black"
        )
        plt.title("Graph Representation with Node Features")
        plt.show()

        num_nodes = 10
        frobenius_norm = (torch.norm(A.unsqueeze(0), p='fro', dim=(1, 2), keepdim=True) ** 2).squeeze(-1).squeeze(-1)
        var = torch.mean(torch.var(A.unsqueeze(0), dim=2), dim=1)
        sec_eig_upperbound = (num_nodes / (num_nodes - 1)) ** 2 * (frobenius_norm - num_nodes ** 2 * var).mean()
        print(sec_eig_upperbound)
        print(A)
        print(np.round(n_f[:10, 8], decimals=1))
        action_feature = env.get_action_feature()  # 차원 : action_size X n_action_feature
...
def main():
    ...
    gamma2 = float(os.environ.get("gamma2", 1.0))
    ...
    agent.load_model("output/episode5471_t_250016_win_0.46875.pt")  #
    t = 0
    epi_r = []
    win_rates = []
    lap_quad = []
