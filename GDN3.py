...
class Agent(nn.Module):
    ...
    def learn(self, cum_losses_old, graph_learning_stop):
        ...
        class Replay_Buffer:
            ...
            def save_buffer(self):
                buffer_dict = {'buffer': self.buffer, 'step_count_list': self.step_count_list,
            ...
            def sample(self):
                ...
                action_feature_next = self.generating_mini_batch(self.buffer, sampled_batch_idx,
                                                                 cat='action_feature_next')
                action_features_next = list(action_feature_next)

                edge_index_enemy_next = self.generating_mini_batch(self.buffer, sampled_batch_idx,
                                                                   cat='edge_index_enemy_next')
                edge_indices_enemy_next = list(edge_index_enemy_next)

                edge_index_ally_next = self.generating_mini_batch(self.buffer, sampled_batch_idx,
                                                                  cat='edge_index_ally_next')
                edge_indices_ally_next = list(edge_index_ally_next)
                avail_action_next = self.generating_mini_batch(self.buffer, sampled_batch_idx, cat='avail_action_next')
                avail_actions_next = list(avail_action_next)
                ...
                agent_feature_next = self.generating_mini_batch(self.buffer, sampled_batch_idx,
                                                                cat='agent_feature_next')
                agent_feature_next = list(agent_feature_next)
        ...
        class Agent(nn.Module):
            def __init__(self,
                ...
                self.func_obs = GAT(feature_size=self.n_representation_obs,
                                    graph_embedding_size=self.graph_embedding).to(
                    device)
                self.func_obs_tar = GAT(feature_size=self.n_representation_obs,
                                        graph_embedding_size=self.graph_embedding).to(
                    device)
                ...
                self.optimizer = optim.RMSprop(param_groups, lr=learning_rate)
                self.scheduler = StepLR(optimizer=self.optimizer, step_size=cfg.scheduler_step,
                                        gamma=cfg.scheduler_ratio)
            ...
            def learn(self, cum_losses_old, graph_learning_stop):
                ...
                obs, A, logits = self.get_node_representation_temp(node_features, agent_feature, edge_indices_enemy,
                                                                   n_agent=n_agent,
                                                                   mini_batch=True, A_old=A)
                obs_next = self.get_node_representation_temp(node_features_next, agent_feature_next,
                                                             edge_indices_enemy_next,
                                                             n_agent=n_agent,
                                                             mini_batch=True, target=True, A_old=A_next)
                ...
                # print(gamma1, gamma2)
                loss = rl_loss + graph_loss + float(
                    os.environ.get("var_reg", 0.5)) * var_+0.001*comm_loss.mean()#######
