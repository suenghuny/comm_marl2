...
class Agent(nn.Module):
    def __init__(self,
        ...
        self.func_comm = GAT(feature_size=self.graph_embedding+self.n_representation_comm, graph_embedding_size=self.graph_embedding_comm).to(device)
        self.func_comm_tar = GAT(feature_size=self.graph_embedding+self.n_representation_comm, graph_embedding_size=self.graph_embedding_comm).to(device)
        self.func_comm2 = GAT(feature_size=self.graph_embedding_comm, graph_embedding_size=self.graph_embedding_comm).to(
            device)
        self.func_comm2_tar = GAT(feature_size=self.graph_embedding_comm, graph_embedding_size=self.graph_embedding_comm).to(
            device)
    ...
    def get_node_representation_temp(self, node_feature, agent_feature, edge_index_obs,n_agent,
        ...
        if mini_batch == False:
            with torch.no_grad():
                ...
                A, _ = self.func_glcn(cat_embedding, mini_batch = False)
                cat_embedding = self.func_comm(X=cat_embedding, A=A, mini_batch=mini_batch, dense = True)
            ...
            if target == False:
                ...
                A_new, logits = self.func_glcn.cal_log_prob(cat_embedding, A_old)

                cat_embedding= self.func_comm(X = cat_embedding, A = A_new, mini_batch = mini_batch, dense = True)

                cat_embedding= self.func_comm2(X = cat_embedding, A = A_new.detach(), mini_batch = mini_batch, dense = True)
                ...
                with torch.no_grad():
                    ...
                    cat_embedding= self.func_comm_tar(X=cat_embedding, A=A_new, mini_batch=mini_batch, dense = True)
                    cat_embedding = self.func_comm2_tar(X=cat_embedding, A=A_new, mini_batch=mini_batch, dense=True)
    ...
    def cal_Q(self, obs, actions, action_features, avail_actions_next, A, target = False):
        ...
        if target == False:
            ...
            q = torch.gather(Q_empty, 2, act_n)                   # q.shape : (batch_size, num_agent, action_size)
            return q
        else:
            with torch.no_grad():
                action_features = torch.tensor(action_features).to(device=device, dtype=torch.float32)
                ...
                for agent_id in range(self.num_agent):
                    ...
                    q_tar = self.Q_tar(obs_and_action)  # batch_size X action_size
                    ...
                    Q_empty[:, agent_id, :] = q_tar
                q_tar_max = torch.max(Q_empty, dim = 2)[0]
                return q_tar_max
