...
class Agent(nn.Module):
    def __init__(self,
        ...
        self.Q1 = Q_Attention(self.graph_embedding_comm, self.n_representation_action, hidden_size_Q).to(device)
        self.Q1_tar = Q_Attention(self.graph_embedding_comm, self.n_representation_action, hidden_size_Q).to(device)
        self.Q1_tar.load_state_dict(self.Q1.state_dict())

        self.Q2 = Q_Attention(self.graph_embedding_comm, self.n_representation_action, self.n_representation_action,
                              hidden_size_Q).to(device)
        self.Q2_tar = Q_Attention(self.graph_embedding_comm, self.n_representation_action, self.n_representation_action,
                                  hidden_size_Q).to(device)
    ...
    def get_node_representation_temp(self, node_feature, agent_feature, edge_index_obs,n_agent,
        ...
        if mini_batch == False:
            ...
            if target == False:
                ...
                cat_embedding= self.func_comm(X = cat_embedding, A = A_old, mini_batch = mini_batch, dense = True)
                ...
                with torch.no_grad():
                    ...
                    cat_embedding = torch.cat([node_embedding_obs, node_embedding_comm], dim=2)
                    cat_embedding= self.func_comm_tar(X=cat_embedding, A=A_old, mini_batch=mini_batch, dense = True)
    ...
    def cal_Q(self, obs, actions, action_features, i = None):
        ...
        if i == 1:
            Q = self.Q1(obs, action_embedding)
        else:
            Q = self.Q2(obs, action_embedding)
    ...
    def cal_Q_tar(self, obs, action_features, avail_actions, i=None):
        with torch.no_grad():
            ...
            Q1_tar = self.Q1_tar(obs, action_embedding)
            Q2_tar = self.Q2_tar(obs, action_embedding)
    ...
    def sample_action(self, node_representation, action_feature, avail_action, epsilon):
        ...
        obs = node_representation.unsqueeze(0)
    ...
    def learn(self, cum_losses_old, graph_learning_stop):
        ...
        td_target = rewards.unsqueeze(1)*self.num_agent + self.gamma* (1-dones.unsqueeze(1))*(q_tot_tar-0.2*torch.log(probs_next))
        probs, min_q = self.cal_prob(obs, actions, action_features, avail_actions)
        pi_loss = torch.sum(min_q-0.2* torch.log(probs), dim = 1).mean()
