...
def gumbel_sigmoid(logits: Tensor, tau: float = 1, hard: bool = True, threshold: float = 0.5,
...
class GLCN(nn.Module):
    ...
    def cal_log_prob(self, h, A_old):
        ...
        mask = ~torch.eye(n, dtype=torch.bool, device=logits_for_improvements.device).unsqueeze(0).expand(
            batch_size, -1, -1)
        selected_probs = A_old * logits_for_improvements + (1 -A_old) * (1 - logits_for_improvements)
