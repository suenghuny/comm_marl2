...
def evaluation(env, agent, num_eval):
    ...
    for e in range(num_eval):
        ...
        while (not done) and (step < max_episode_len):
            ...
            agent_feature = torch.concat([torch.tensor(sum_state, dtype = torch.float).repeat(num_agent, 1), torch.tensor(node_feature, dtype = torch.float)[:num_agent, :]],dim=1)

            #agent_feature = torch.concat([torch.tensor(sum_state, node_feature)[:num_agent,:], action_history.to('cpu')], dim = 1)
            #agent_feature = torch.tensor(node_feature)[:num_agent, :]#torch.concat([torch.tensor(node_feature)[:num_agent, :-1], action_history.to('cpu')], dim=1)
...
def main():
    ...
    gamma1 = float(os.environ.get("gamma1", 3.0))#
