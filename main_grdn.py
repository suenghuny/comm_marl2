...
def evaluation(env, agent, num_eval):
    ...
    for e in range(num_eval):
        ...
        while (not done) and (step < max_episode_len):
            ...
            node_representation, A, probs = agent.get_node_representation_temp(
                                                                 node_feature,
                                                                 agent_feature,
                                                                 edge_index_enemy,
                                                                n_agent=n_agent,
            ...
            action, action_history = agent.sample_action(node_representation, action_feature, avail_action, epsilon=0, A = A)
...
def train(agent, env, e, t, train_start, epsilon, min_epsilon, anneal_epsilon, initializer, cum_losses_old, graph_learning_stop, anneal_gamma2, min_gamma2):
    ...
    while (not done) and (step < max_episode_limit):
        ...
        node_representation, A, probs = agent.get_node_representation_temp(
            ...
            mini_batch=False)
        action_feature = env.get_action_feature()  # 차원 : action_size X n_action_feature
        action, action_history = agent.sample_action(node_representation, action_feature, avail_action, epsilon, A)
        reward, done, info = env.step(action)
        agent.buffer.memory(node_feature, action, action_feature, edge_index_enemy, A, reward, done, avail_action, dead_masking, agent_feature.tolist(), probs)
        episode_reward += reward
        ...
        if e >= train_start:
            if cfg.given_edge == True:
                ...
                q_tots.append(q_tot)
                comm_losses.append(comm_loss)
            losses.append(loss.detach().item())
        if epsilon >= min_epsilon:
            epsilon = epsilon - anneal_epsilon
        else:
            epsilon = min_epsilon
    if agent.gamma2 >= min_gamma2:
        agent.gamma2 = agent.gamma2 - anneal_gamma2
    else:
        agent.gamma2 = agent.gamma2

    print("{} Total reward in episode {} = {}, epsilon : {}, time_step : {}, episode_duration : {}, gamma2 : {}".format(env.map_name,e,np.round(episode_reward, 3),np.round(epsilon, 3),t, np.round(time.time()-start, 3), agent.gamma2))
    if cfg.given_edge == True:
...
def main():
    ...
    graph_embedding_comm = int(os.environ.get("graph_embedding_comm", 56))
    ...
    gamma1 = float(os.environ.get("gamma1", 1.0))#
    gamma2 = float(os.environ.get("gamma2", 2.0))
    min_gamma2 = 10.0
    ...
    anneal_gamma2 = (gamma2- min_gamma2) / 10000
    ...
    for e in range(num_episode):
        ...
        if e % 10 == 1:
            if vessl_on == True:
                ...
                print(np.mean(comm_losses))
                wandb.log({
                    "episode":e,
                    "reward": np.mean(epi_r),
                    "lap_quad": np.mean(lap_quad),
                    "sec_eig": np.mean(sec_eig),
                    "rl_lo": np.mean(rl_lo),
                    "q_t": np.mean(q_t),
                    'comm_loss': np.mean(comm_losses)
